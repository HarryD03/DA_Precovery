...
def main():
    ...
    XI = array.zeros(4)      #Initialise initial state
    xb=0.008                 #uncertanity in x
    yb=0.08                  #uncertanity in y
    XI[0] += 1.0              #x=1
    ...
    lb[:, 0] = lb[:,0]*(-xb)            # 1 * -0.008
    lb[:, 1] = yb*ygrid                 # 0.08 * y grid points
    ...
    #Normalise the perimeter so the DA space boundary is [-1,1]
    ...
    try:
        ...
        #DA initialization of initial conditions - uncertanity only in initial position (i.e measurement error)
        XI = array.zeros(4)             #Initialise state variables array
        XI[0] += 1.0 + xb * DA(1)       # x = 1 + uncertanity (xb*DA(1))
        XI[1] += 0.0 + yb * DA(2)       # y = 0 + uncertanity (yb*DA(2))
        XI[3] += np.sqrt(1.5)

        XFN = array.zeros((4,Ts))       #Function output array initialisation: 4 components, Ts timesteps
        XFN[:, 0] = XI.copy()           #Initial condition

    # with DA.cache_manager():  # optional, for efficiency
        x0 = array(XI)                  #Initial condition
        xf = array(XI)                  #Propagated condition
        for i in range(Ts-1):           #For each timestep
            t0 = tgrid[i]               
            tf = tgrid[i+1]
            xf = RK78(x0, t0, tf, TBP)  #Propagate state i to i+1 using RK78
            XFN[:, i+1] = xf.copy()     #Store propagated state: STILL A FUNCTION OF DA VARIABLES

            x0 = xf                     #reinitialise initial condition
        ...
        for i in range(Ts):                                                 #for each timestep
            ...
            for j in range(perimeter_norm.shape[0]):                        #for each point on the perimeter
                XF1[:, i, j] = xf_temp.eval(perimeter_norm[j, :])           #evaluate the propagated state at the perimeter point
        DA.popTO()
