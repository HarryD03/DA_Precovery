import pytest
import numpy as np
from utils.time_reference import J0, zeroTo360, LST, equatorial_to_eclipitcJ2000, create_da_los_vectors, CC2COE, CC2MEE, MEE2CC, COE2CC, ROT1, ROT3, CC2obs, obs2CC, dms_to_degrees
...
def test_dms_to_degrees():
    # Test DMS to degrees conversion
    dms = np.array([[-30, 15, 50]])  # -30 degrees, 15 minutes, 50 seconds
    degrees = dms_to_degrees(dms)
    expected = -30 + 15/60 + 50/3600
    assert np.isclose(degrees, expected, rtol=1e-6), (
        f"Mismatch: Expected {expected}, got {degrees}"
    )